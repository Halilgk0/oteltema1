...
@section('title', 'Yeni Oda Ekle')

@section('content')
    <div class="bg-white rounded-lg shadow-md">
        <div class="p-6 border-b border-gray-100">
            <h2 class="text-lg font-semibold">Yeni Oda Ekle</h2>
        </div>
        <div class="p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="is_clean" id="is_clean" 
                                class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring focus:ring-blue-200"
                                required>
                            <option value="1" {{ old('is_clean') == '1' ? 'selected' : '' }}>Temiz</option>
                            <option value="0" {{ old('is_clean') == '0' ? 'selected' : '' }}>Temizlik Gerekli</option>
                        </select>
                        @error('is_clean')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
